...
function deploy(consul, statusEntry, nodes) {
  ...
  .on('finish', () => {
    async.eachSeries(nodes, (node, cb) => {
      ...
      .then(() => {
        // Remove app process
        return s.execCommand('pm2 delete all');
      })
      .then(() => {
        // Copy zip
      ...
      .then(() => {
        // Run install deps
        return s.execCommand('sudo npm i --production', { cwd: `/opt/current-app/${repoInfo.repo}-${repoInfo.branch}`});
      })
      .then(() => {
        // Run app
        return s.execCommand('pm2 start npm -- start', { cwd: `/opt/current-app/${repoInfo.repo}-${repoInfo.branch}`});
      })
      .then(() => {
        // Add to Consul
